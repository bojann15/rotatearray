...
const Elements = () => {
    ...
    useEffect(() => {
        ...
        if(songs.length === 0){
            interval = setInterval(() =>{ setElements( rotate(elements));
            }, 1000);
        } else{
            interval = setInterval(() =>{ setElements( changeEL(elements, songs));
            }, 1000); 
        }
    ...
    const handleSearch = async (e) => {
        ...
        if(search.length !== 0 && search !== ''){
            const response = await API.get(`https://itunes.apple.com/search?term=${search}`);
